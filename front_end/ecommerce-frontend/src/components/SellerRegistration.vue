<!-- components/SellerRegistration.vue -->
<template>
      ...
      <transition name="box-fade" mode="out-in">
        ...
        >
          <div class="mb-8 text-center">
            <h1 class="text-3xl font-playfair text-emerald-900">Become a Seller</h1>
            <p class="mt-2 text-stone-600 font-light">Register as a seller to start selling on our platform</p>
          </div>
          
          <form @submit.prevent="submitSellerForm" class="space-y-6">
            <!-- Username Field -->
            <div class="space-y-2">
            ...
            <!-- Business Name Field -->
            <div class="space-y-2">
              <label for="businessName" class="block text-sm font-medium text-emerald-900 font-libre">Business Name</label>
              <div class="relative">
                <input 
                  type="text" 
                  id="businessName" 
            ...
            <!-- Tax ID Field -->
            <div class="space-y-2">
              <label for="taxId" class="block text-sm font-medium text-emerald-900 font-libre">Tax ID</label>
              <div class="relative">
                <input 
                  type="text" 
                  id="taxId" 
            ...
            >
              ...
              <span v-else>Register as Seller</span>
            </button>
  ...
  const submitSellerForm = async () => {
    ...
    try {
      ...
      console.log('Seller Data:', sellerData); // Log the payload

      await authStore.registerSeller(sellerData);
      successMessage.value = 'Seller registration submitted for approval!';
      router.push('/');
    } catch (error) {
      errorMessage.value = error.response?.data?.error || 'Error registering as seller. Please try again.';
    } finally {
      loading.value = false;
    }
  };
