...
//               quantity: Number(item.quantity) || 0,
//               productId: item.productId,
//               imageUrl: product.imageUrl, // Add imageUrl here
//               productDetails: product, // Add all product details if needed
//             };
//           } catch (productError) {
  ...
  getters: {
    ...
    cartItems: (state) => {
      return state.items.map(item => ({
        ...item,
        // Ensure image URL is always available
        imageUrl: item.imageUrl || item.productDetails?.imageUrl || '/placeholder-product.jpg'
      }));
    }
  },
      ...
      try {
        ...
        if (savedCart) {
          ...
          this.items = JSON.parse(savedCart).map(item => ({
            ...item,
            imageUrl: item.imageUrl || item.productDetails?.imageUrl || '',
        ...
        this.items = await Promise.all(response.data.map(async item => {
          try {
            ...
            return {
              ...
              productId: item.productId,
              imageUrl: product.imageUrl, // Add imageUrl here
              productDetails: product // Add all product details
            };
          } catch (productError) {
            console.error(`Error fetching product ${item.productId}:`, productError);
            return {
